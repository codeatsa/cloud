...
public class Main {
    ...
    public static void launchMainFrame() {
        ...
        JFrame frame = new JFrame("Cloud Resource Management System");
        ...
        // Launch QNPSO Results Frame
//        QNPSOResultsFrame qnpsoFrame = new QNPSOResultsFrame();
//        qnpsoFrame.setVisible(true);

        // Open Create Host Window
...
private static String getLoadCategory(int value) {
  ...
  private static void runQNPSO(VMAllocation vm, JTextArea hostDisplay) {
    if (qnpsoFrame == null || !qnpsoFrame.isDisplayable()) {
        ...
        // Create output area with better formatting
...
private static JPanel createVisualizationPanel() {
    ...
    // Add different load types with progress bars
...
private static Color getLoadColor(double load) {
    if (load > 100) return new Color(231, 76, 60);  // Red for high load
    if (load > 50) return new Color(241, 196, 15);  // Yellow for medium load
    return new Color(46, 204, 113);                 // Green for low load
}
...
private static void openChartFrame(VMAllocation vm) {
    if (chartFrame == null || !chartFrame.isDisplayable()) {
        ...
        chartFrame.setState(JFrame.NORMAL); // Restore if minimized
        chartFrame.toFront(); // Bring to front
...
private static void exportMetricsData(Document document) throws Exception {
    ...
    private static void updateHostDisplay(JTextArea hostDisplay, VMAllocation vmAllocation) {
        ...
        for (PowerHost host : vmAllocation.dt.hostList) {
            ...
            int usedCpu = 0;  // Corrected: initialize the used CPU to 0

            // Calculate used CPU by checking the CPU cores allocated to VMs on this host
            for (PowerVm vm : vmAllocation.dt.vmlist) {
                if (vm.getHost() != null && vm.getHost().getId() == host.getId()) {
                    usedCpu += vm.getNumberOfPes();  // Add the CPU cores allocated to this VM
                }
            }
